...
def reciprocal_rank(y_true, y_pred) -> float:
    rank = 1
    y_true_ = set(y_true)
    for y_p in y_pred:
        if y_p in y_true_:
...
def mean_reciprocal_rank(y_true, y_pred) -> float:
...
def discounted_cumulative_gain(relevances) -> float:
    dcg_value = 0.0
...
def ndcg_at_k(y_true, y_pred, k=10) -> float:
