...
class NearestNeighborsClassification(BaseModel):
    def __init__(self, k=1, distance='euclidean', verbose=False):
        super(NearestNeighborsClassification, self).__init__(verbose=verbose)
        self.k = k
    ...
    def predict(self, X):
        if self.distance == 'euclidean':
            ...
            for x in X:
                ...
                y_hat.append(np.round(np.average(self.y[neighbors_indexes])))
            return y_hat
        else:
